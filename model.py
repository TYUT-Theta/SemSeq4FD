...
from layers import GraphConvolution, GraphAttentionLayer, Lstmen1, TextCNN
import matplotlib
...
class Classify(torch.nn.Module):
    def __init__(self, params, vocab_size, ntags, pte=None):
        ...
        if params.encoder == 2:
            ...
            self.cnn1 = TextCNN(params.hidden_dim, params.node_emb_dim, [3])
            self.lstm1 = Lstmen1(params.node_emb_dim, params.node_emb_dim * 2)

            # self.gcn2 = GraphAttentionLayer(params.hidden_dim, params.node_emb_dim, params.dropout, 0.2)
            #
    ...
    def forward(self, input_sents, input_lens, adj, actual_sentence=None):
        ...
        if self.params.encoder==0:
            h3=h
        if self.params.encoder==1:
            h3=h
        if self.params.encoder == 2:
            ...
            h1 = self.gcn1(h, adj_matrix)  # gcn卷积后聚合了邻居信息的h
            att1 = F.softmax(torch.mm(h1, h1.transpose(0, 1)) / np.sqrt(self.params.node_emb_dim), dim=1)
            h1 = torch.mm(att1, h1)
            h2 = torch.unsqueeze(h, dim=0)
            h2 = self.cnn1(h2)
            h3 = torch.cat((h1, h2), dim=1)
            h3 = torch.unsqueeze(h3, dim=0)
            h3 = self.lstm1(h3, [h3.shape[1]])
            h3, _ = h3.data.max(
                dim=0)  # 按维度dim 返回最大值，并且返回索引。dim=0返回列方向上最大值的那个元素，且返回索引（返回最大元素在这一列的行索引）。返回的最大值和索引各是一个tensor，一起构成元组(Tensor, LongTensor)
            ...
            # Simple max pool on all node representations
            h3, _ = h.max(dim=0)
        h3 = self.linear_transform(h3)  # bs * ntags
        return h3
